      ...
      <tbody>
        <tr v-for="order in filterdOrders" :key="order.awb">
         <td>
          ...
          <td class="actions">
            <button @click="this.$store.dispatch('SetDeliverReady',order)" v-if="order.status == 5">{{__('ReadyToDeliver')}}</button>
            <button @click="this.$store.dispatch('SetReturnReady',order)" v-if="order.status == 20">{{__('ReadyToReturn')}}</button>
          </td>
...
export default {
  ...
  methods:{
    ...
    addNote(v){
      ...
      this.$genStore.dispatch('orderNote', {id:this.addNoteToggler,note:v})
